...
int main(int argc, char **argv) {
    ...
    // ros::Publisher pubOdomAftMapped = nh.advertise<nav_msgs::Odometry>("/aft_mapped_to_init",
    // 10);
    ...
    ros::Publisher pubLoopConstraintEdge =
            nh.advertise<visualization_msgs::MarkerArray>("/loop_closure_constraints", 10);

    ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>("/cloud_registered_body",
                                                                           100, laserCloudHandler);
            ...
            *key_cloud += *current_cloud_world;
            ...
            if (!cloudInd) {
                ...
                graph.add(gtsam::BetweenFactor<gtsam::Pose3>(
                        cloudInd - 1, cloudInd, prev_pose.between(curr_pose), odometryNoise));
            }
            ...
            if (cloudInd % config_setting.sub_frame_num_ == 0 && cloudInd != 0) {
                ...
                if (search_result.first > 0) {
                    ...
                    // std::cout << "rotation(ypr): " << euler[0] << ' ' << euler[1] << ' ' << euler[2]
                    //           << std::endl;
            ...
            pcl::transformPointCloud(*current_cloud_body, correct_cloud, latest_estimate_affine3d);
            sensor_msgs::PointCloud2 pub_cloud;
            pcl::toROSMsg(correct_cloud, pub_cloud);
            pub_cloud.header.frame_id = "camera_init";
            pubCorrectCloud.publish(pub_cloud);

            Eigen::Quaterniond _r(latest_estimate_affine3d.rotation());
            nav_msgs::Odometry odom;
            odom.header.frame_id = "camera_init";
            odom.pose.pose.position.x = latest_estimate_affine3d.translation().x();
            odom.pose.pose.position.y = latest_estimate_affine3d.translation().y();
            odom.pose.pose.position.z = latest_estimate_affine3d.translation().z();
